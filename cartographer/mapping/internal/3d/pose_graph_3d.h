...
class PoseGraph3D : public PoseGraph {
 public:
  ...
  sensor::MapByTime<sensor::FixedFramePoseData> GetFixedFramePoseData()
      const override (mutex_);
  std::map<std::string /* landmark ID */, PoseGraph::LandmarkNode>
  GetLandmarkNodes() const override (mutex_);
  std::map<int, TrajectoryData> GetTrajectoryData() const override;

  std::vector<Constraint> constraints() const override (mutex_);
 ...
 protected:
  ...
  void WaitForAllComputations() (mutex_)
      (work_queue_mutex_);
 ...
 private:
  ...
  NodeId AppendNode(
      ...
      int trajectory_id,
      const std::vector<std::shared_ptr<const Submap3D>>& insertion_submaps,
      const transform::Rigid3d& optimized_pose) (mutex_);
  ...
  WorkItem::Result ComputeConstraintsForNode(
      ...
      bool newly_finished_submap) (mutex_);
  ...
  // constraint search.
  void DeleteTrajectoriesIfNeeded() (mutex_);
  ...
  void DrainWorkQueue() (mutex_)
      (work_queue_mutex_);

  // Runs the optimization. Callers have to make sure, that there is only one
  // optimization being run at a time.
  void RunOptimization() (mutex_);
  ...
  transform::Rigid3d ComputeLocalToGlobalTransform(
      ...
      int trajectory_id) const (mutex_);
  ...
  // poses.
  void LogResidualHistograms() const (mutex_);
  ...
  absl::flat_hash_map<int, std::unique_ptr<common::FixedRatioSampler>>
      global_localization_samplers_ (mutex_);

  // Number of nodes added since last loop closure.
  int num_nodes_since_last_loop_closure_ (mutex_) = 0;
  ...
  std::vector<std::unique_ptr<PoseGraphTrimmer>> trimmers_ (mutex_);

  PoseGraphData data_ (mutex_);
